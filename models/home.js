...
module.exports = class Home {
  ...
  if (this.id) { // update
    return db.execute(
      'UPDATE homes SET house_name=?, price=?, location=?, rating=?, photo_url=?, description=? WHERE id=?',
      [this.houseName, this.price, this.location, this.rating, this.photoUrl, this.description, this.id]
    );
  } else { // insert
    return db.execute(
      'INSERT INTO homes (house_name, price, location, rating, photo_url, description) VALUES (?, ?, ?, ?, ?, ?)',
